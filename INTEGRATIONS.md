...
| Payments | Stripe Checkout via Supabase Edge Functions (create-checkout-session + stripe-webhook, deployed) | LIVE PENDING SECRETS: products, prices and webhook exist in the live account; PAYMENTS_LIVE is true; the owner must still set the Edge Function secrets and confirm charges are enabled (supabase/README.md) | Never handle card data in the app; Stripe-hosted checkout only. Secrets never enter the repo. |

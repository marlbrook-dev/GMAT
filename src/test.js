const fs=require('fs');
const src=['bank_quant.js','bank_quant2.js','bank_quant3.js','bank_verbal.js','bank_verbal2.js','bank_verbal3.js','bank_di.js','bank_di2.js','bank_di3.js','cards.js','engine.js'].map(f=>fs.readFileSync(f,'utf8')).join('\n');
const vm=require('vm'); const ctx={console,Date,Math,JSON,Set}; vm.createContext(ctx);
vm.runInContext(src+`
var BANK=[].concat(BANK_QUANT,BANK_QUANT2,BANK_QUANT3,BANK_VERBAL,BANK_VERBAL2,BANK_VERBAL3,BANK_DI,BANK_DI2,BANK_DI3);
console.log('bank',BANK.length,'Q',BANK_QUANT.length+BANK_QUANT2.length+BANK_QUANT3.length,'V',BANK_VERBAL.length+BANK_VERBAL2.length+BANK_VERBAL3.length,'DI',BANK_DI.length+BANK_DI2.length+BANK_DI3.length);

...
def consent_js():
    ...
    return (
        ...
        "  // Same rejection trap as the error beacon: catch the promise, not the call.\n"
        "  try{ fetch(U,{method:'POST',headers:{'Content-Type':'application/json'},"
        "body:JSON.stringify(o),keepalive:true}).catch(function(){}); }catch(e){} }\n"
...
def sentinel_js(app_version=None):
    ...
    return (
        ...
        " // fetch REJECTS on a network failure, it does not throw, so the try/catch\n"
        " // around it caught nothing and the rejected promise fell through to the\n"
        " // unhandledrejection handler below, which called report again. That loop put\n"
        " // 267 Failed to fetch rows in the table, one per page a crawler swept. The\n"
        " // .catch is what makes the no-self-reporting rule above actually true.\n"
        " try{fetch(U,{method:'POST',headers:{'Content-Type':'application/json'},"
        "body:JSON.stringify(o),keepalive:true}).catch(function(){});}catch(e){} }\n"
        ...
        "  // Only our own files. A third party script blocked by an ad blocker or a\n"
        "  // privacy extension is not a bug we can fix, and it is not rare: 1,202 of\n"
        "  // the first 1,490 rows here were one blocked Cloudflare beacon, which left\n"
        "  // the real errors at 19 percent of a table nobody could read.\n"
        "  if(u.indexOf(location.origin)!==0 && u.charAt(0)!=='/') return;\n"

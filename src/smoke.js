...
(async()=>{
 ...
 for(let i=0;i<9;i++){ const t=await p.$('.tpa'); const gi=await p.$('.gi-st select'); const ta=await p.$('table.ta');
  if(t){ await p.click('input[name=tpa0]'); await p.click('input[name=tpa1]'); }
  else if(gi){ for(const s of await p.$$('.gi-st select')) await s.selectOption({index:1}); }
  else if(ta){ for(const r of await p.$$('table.ta tbody tr')) await (await r.$('input')).click(); }
  else { await p.click('#opt1'); } await p.click('#submitBtn'); await p.waitForTimeout(100); const rb=await p.$('.reason button'); if(rb) await p.click('.reason button'); const nx=await p.$('text=Next question'); if(nx) await nx.click(); else { await p.click('text=See results'); } await p.waitForTimeout(100); }
 ...
 await p.click('nav >> text=Drill'); await p.waitForTimeout(200); await p.click('nav >> text=Playbook'); await p.click('nav >> text=Account'); await p.waitForTimeout(200);

...
- python3 src/build.py builds everything (both trainer apps, landing, rankings, exams, pricing), node-parses every inline script, and fails on an em or en dash in any hand-edited doc or bank; cd src && node test.js runs engine tests once per exam; validate in headless Chromium (executablePath /opt/pw-browsers/chromium-*/chrome-linux/chrome) before shipping.
...
- SAT honesty rules: never report a 400 to 1600 score (College Board's equating tables are not public), never present our 60 percent module routing cut as official, and keep every structural fact sourced (see the Exam Content Sources section of data/DATA.md).
